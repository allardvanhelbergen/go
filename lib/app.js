...
App.prototype.start = function() {
    if (!this.started) {
        ...
        this.app.listen.bind(this, this.app.get('port'), function() {
            fs.readFile.bind(this, path.join(__dirname, '..', 'fixtures', 'LAUNCH_MESSAGE.txt'), 'utf-8', function(err, data) {
                if (err) {
                    return winston.error('Read file error:', err);
                }
                winston.warn(data);
                winston.warn('App running in %s mode.', this.app.get('env'));
                winston.warn('Server listening on http://localhost:%d.', this.app.get('port'));
            });
        });
    }
}
